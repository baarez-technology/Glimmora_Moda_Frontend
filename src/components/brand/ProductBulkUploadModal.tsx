...
import { uploadMultipleProducts, downloadProductSample } from '@/services/brand-product.service';
import type { ProductImportResult } from '@/services/brand-product.service';
...
export default function ProductBulkUploadModal({ onClose, onSuccess }: ProductBulkUploadModalProps) {
  ...
  const handleUpload = async () => {
    ...
    setErrorMessage(null);
    try {
      const res = await uploadMultipleProducts(selectedFile);
      setResult(res);
  ...
  const handleDownloadSample = async (format: SampleFormat) => {
    ...
    try {
      await downloadProductSample(format);
    } catch {
      // silently ignore
    } finally {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-noir/50 p-4">
      <div className="bg-white w-full max-w-xl border border-sand shadow-xl">
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-sand/50">
          <div>
            <h2 className="font-display text-base text-charcoal-deep tracking-wide">Bulk Import Products</h2>
            <p className="text-xs text-stone mt-0.5">Upload multiple products at once via JSON, CSV, or Excel</p>
          </div>
        ...
        <div className="p-6 space-y-5">
            ...
            <div className="p-4 bg-success/5 border border-success/20 space-y-2">
              ...
              <div className="pl-6 space-y-1">
                <p className="text-xs text-stone">
                  <span className="font-medium text-charcoal-deep">{result.imported_count ?? 0}</span> product{result.imported_count !== 1 ? 's' : ''} created
                </p>
                {result.product_ids && result.product_ids.length > 0 && (
                  <p className="text-[11px] text-taupe font-mono break-all">
                    IDs: {result.product_ids.slice(0, 3).join(', ')}{result.product_ids.length > 3 ? ` +${result.product_ids.length - 3} more` : ''}
                  </p>
                )}
              </div>
            </div>
          ...
          <p className="text-[11px] text-taupe leading-relaxed">
            Each row (or object in JSON) represents one product. Required:{' '}
            <span className="font-medium text-stone">product_name</span>,{' '}
            <span className="font-medium text-stone">sku</span>,{' '}
            <span className="font-medium text-stone">price</span>.{' '}
            Optional:{' '}
            <span className="font-medium text-stone">collection_name</span>,{' '}
            <span className="font-medium text-stone">product_category</span>,{' '}
            <span className="font-medium text-stone">tagline</span>,{' '}
            <span className="font-medium text-stone">product_description</span>,{' '}
            <span className="font-medium text-stone">sizes</span> (JSON array),{' '}
